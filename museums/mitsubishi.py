...
def get_mimt_exhibitions():
    ...
    # 「開催中の展覧会」が明示されている見出しやセクションを探す
    sections = soup.find_all("section")

    for section in sections:
        h2 = section.find("h2")
        if h2 and "開催中の展覧会" in h2.get_text():
            # 中に展覧会情報が含まれている場合
            items = section.find_all("article")
            for item in items:
                exb = {}
                ...
                if a_tag and "href" in a_tag.attrs:
                    ...
                    if not detail_url.startswith("http"):
                        detail_url = "https://mimt.jp" + detail_url
                    ...
                    try:
                        ...
                        for text in fee_text_candidates:
                            parent = text.find_parent(["p", "div", "li", "td"])
                            if parent:
...
if __name__ == "__main__":
    exhibitions = get_mimt_exhibitions()
    if not exhibitions:
